    ...
    <div class="layout">
      ...
      <main class="content">
        <Transition name="tab" mode="out-in">
          ...
          <section v-else-if="currentTab === 'github'" class="tab-section" key="github">
            <GithubNotificationsTab />
          </section>

          <!-- ── Weekly Reports Tab ── -->
          <section v-else-if="currentTab === 'reports'" class="tab-section" key="reports">
            <WeeklyReportsTab />
          </section>
            ...
            <div class="settings-block danger-block">
              ...
              <div v-if="deletionStep === 'idle'" class="inline-actions">
                <button class="btn-danger" @click="handleRequestDeletion" :disabled="deletionLoading">
                  <span v-if="deletionLoading" class="btn-loading"><span></span><span></span><span></span></span>
...
import GithubNotificationsTab from '../components/GithubNotificationsTab.vue'
import WeeklyReportsTab from '../components/WeeklyReportsTab.vue'

type Tab = 'profile' | 'security' | 'connections' | 'github' | 'reports' | 'danger'
...
const tabs = [
  ...
  { id: 'github' as Tab, label: 'GitHub', icon: GithubIcon },
  { id: 'reports' as Tab, label: 'Reports', icon: ReportsIcon },
...
.orb-1 { width: 600px; height: 400px; top: -80px; left: -80px; background: radial-gradient(ellipse, rgba(200,169,126,0.045) 0%, transparent 70%); animation: orbFloat 16s ease-in-out infinite; }
.orb-2 { width: 500px; height: 500px; bottom: -120px; right: -80px; background: radial-gradient(ellipse, rgba(90,80,170,0.035) 0%, transparent 70%); animation: orbFloat 20s ease-in-out infinite reverse; }
.grid-overlay { position: absolute; inset: 0; background-image: linear-gradient(rgba(200,169,126,0.018) 1px, transparent 1px), linear-gradient(90deg, rgba(200,169,126,0.018) 1px, transparent 1px); background-size: 64px 64px; }
...
.topbar-brand { display: flex; align-items: center; gap: 10px; filter: drop-shadow(0 0 10px rgba(200,169,126,0.2)); }
.brand-text { font-family: var(--font-display); font-size: 11px; font-weight: 600; letter-spacing: 0.45em; text-transform: uppercase; color: var(--accent); }
...
.btn-logout:hover:not(:disabled) { color: var(--text); border-color: rgba(255,255,255,0.14); background: rgba(255,255,255,0.04); }
...
.nav-item.danger { color: rgba(224, 96, 96, 0.65); }
.nav-item.danger:hover { color: var(--error); background: rgba(224,96,96,0.07); }
.nav-item.danger.active { color: var(--error); background: rgba(224,96,96,0.09); }

.nav-item.danger.active::before { background: var(--error); }

@media (max-width: 700px) {
  .nav-item { flex: 1; justify-content: center; border-radius: 0; padding: 12px 8px; font-size: 12px; min-width: 0; }
  ...
  .nav-item.danger.active::after { background: var(--error); }
}
...
.tab-section { display: flex; flex-direction: column; gap: 28px; }
...
.profile-card::before {
  ...
  height: 1px;
  background: linear-gradient(90deg, transparent 0%, rgba(200,169,126,0.3) 40%, rgba(200,169,126,0.3) 60%, transparent 100%);
}
...
.card-name { font-family: var(--font-display); font-size: 22px; font-weight: 400; color: var(--text); margin-bottom: 6px; }
...
.card-fields { display: grid; grid-template-columns: 1fr 1fr; gap: 20px 28px; }
.span-2 { grid-column: span 2; }
.card-field { display: flex; flex-direction: column; gap: 4px; }
.field-value { font-size: 14px; color: var(--text); }
.field-id { font-family: var(--font-mono); font-size: 11px; color: var(--text-2); word-break: break-all; opacity: 0.8; }
...
@keyframes orbFloat    { 0%, 100% { transform: translate(0,0) scale(1); } 33% { transform: translate(28px,-18px) scale(1.04); } 66% { transform: translate(-18px,28px) scale(0.96); } }
</style>
